module mini_haze_i_dlsode_mom_mod
  ...
  subroutine mini_haze_i_dlsode_mom(n_eq, T_in, P_in, mu_in, grav_in, t_end, vf, q)
    implicit none
  ...
  subroutine RHS_mom(n_eq, time, y, f)
    ...
    !! Add a production rate source term for each moment
    call find_production_rate(n_eq, f_prod)
    ...
    !! Add loss term if above given pressure level (pa)
    ...
    !print*, time, y(:), f(:), -f_coal, -f_coag, &
    !  & ((3.0_dp*y(2)/y(1))/(4.0_dp*pi*rho_h))**(1.0_dp/3.0_dp) * 1e4_dp
